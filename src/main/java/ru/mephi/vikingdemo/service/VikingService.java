...
public class VikingService {
    ...
    public List<Viking> generateRandomVikings(int count) {
        List<Viking> generated = IntStream.range(0, count)
                .mapToObj(index -> vikingFactory.createRandomViking(nextId++))
    ...
    public String getRandomTallVikingText() {
        Supplier<List<Viking>> tallVikings = () -> vikings.stream()
    ...
    public String getLegendaryVikingsText() {
        Predicate<Viking> condition = viking -> viking.equipment().stream()
                .anyMatch(item -> item.quality().equals("Legendary"));
        return format(vikings.stream()
                .filter(condition)
                .collect(Collectors.toList()));
    }
    ...
    public String getRedBeardVikingsSortedByAgeText() {
        ...
        return format(vikings.stream()
                .filter(condition)
                .sorted((first, second) -> Integer.compare(first.age(), second.age()))
                .collect(Collectors.toList()));
    }

    public int getMaxId() {
        return vikings.stream()
                .mapToInt(viking -> viking.id())
                .max()
                .orElse(0);
    }
    ...
    private int countAxes(Viking viking) {
        return (int) viking.equipment().stream()
    ...
    private String format(List<Viking> result) {
        ...
        return result.stream()
                .map(viking -> format(viking))
                .collect(Collectors.joining("\n"));
    }

    private String format(Viking viking) {
        Function<Viking, String> formatter = value -> "ID: " + value.id()
                + ", " + value.name()
                + ", возраст: " + value.age()
                + ", рост: " + value.heightCm()
                + ", волосы/борода: " + value.hairColor()
                + ", форма бороды: " + value.beardStyle()
                + ", топоров: " + countAxes(value)
                + ", снаряжение: " + formatEquipment(value.equipment());
